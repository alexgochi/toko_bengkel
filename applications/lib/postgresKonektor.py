...
class PsycopgError(Exception):
    ...
    def __init__(
        self, pgcode: str, pgerror: str, diag: Diagnostics = VOID_DIAG
    ) -> None:
    ...
    def diag(self, value: Diagnostics) -> None:
        if not isinstance(value, Diagnostics):
            raise ValueError(
                f"{value} is not instance of <psycopg2.extensions.Diagnostics>!"
            )
        self._diag = value
...
class DBResponse:
    def __init__(
        ...
        if not isinstance(custom_error, bool):
            raise ValueError("cumba bisa boolean bos!")
        else:
            self.__custom_error = custom_error
    ...
    @property
    def is_custom_error(self) -> bool:
        """
        True bila DBResponse object didapat dari method custom_error
        False jika didapat selain dari method custom_error
        """
        return self.__custom_error
    ...
    def is_empty(self) -> bool:
        ...
        if self.is_error:
            raise ValueError(
                "is_empty hanya bisa dipanggil jika tidak ada error (pgcode == '00000')"
            )
    ...
    def first(self) -> dict:
        ...
        """
        if self.is_error:
            # TODO: pakai caller dong
            raise ValueError(
                "first hanya bisa dipanggil jika tidak ada error (pgcode == '00000')"
            )
    ...
    def to_dict(self) -> dict:
        ...
        """
        return {"pgcode": self.pgcode, "pgerror": self.pgerror, "result": self.result}


class PostgresDatabase:
    @property
    def connection(self):
        if self.__preserveConn:
            self.__connection = self.__preserveConn
            return self.__connection
        else:
            ec = self.__establish_connection()
            if ec.is_error:
                raise Exception(ec.pgerror)
            return self.__connection
    ...
    def __init__(self, connection_key: str = "default") -> None:
        ...
        self.__flag_execute_biasa = False
    ...
    def __establish_connection(self) -> DBResponse:
        ...
        try:
            ...
            # execute biasa tidak bisa dipakai bersamaan dgn *_preserve() ya ges :)
            if self.__preserveConn:
                caller = self.__get_caller(2)
                raise Exception(
                    f"Maaf, method '{caller['name']}()' nggak bisa dicampur sama method dgn prefix *_preserve()!\n"
                )
    ...
    def __generate_preserve_conn(self) -> Tuple[bool, DBResponse]:
        ...
        Hanya boleh pakai 1 jenis execute tiap instance PostgresDatabase:
            - dgn prefix _preserve():
                execute(), execute_many(), execute_dt(), dll
            - tanpa prefix _preserve():
                execute_preserve(), execute_many_preserve(), dll
        Kenapa dibuat aturan gitu?. hmm biar konsisten aja sih, sama menghindari 'keteledoran' programmer-nya (aku pernah soalnya wkwk)
        NOTE: Mungkin perlu dikasih opsi 'force connection' kali ya?, jadi bisa di-mix, kalau error ya salahin yg make?
        """
        if self.__flag_execute_biasa:
            caller = self.__get_caller(2)
            raise Exception(
                f"Maaf, method '{caller['name']}()' nggak bisa dicampur sama method execute() biasa!\n"
            )
    ...
    def __get_caller(self, dept_level: int) -> Dict:
        ...
        caller_info = (
            f'File "{caller_filename}", line {caller_lineno}, in {caller_name}'
        )
    ...
    def __get_notices(self) -> List[str]:
        ...
        return [n.split(": ")[-1].strip() for n in self.__connection.notices]
    ...
    def __serialize_exception(self, e: Exception) -> PsycopgError:
        ...
        if isinstance(e, str):
            e = PsycopgError(pgcode="", pgerror=e)
            ...
            e = PsycopgError(pgcode="", pgerror=f"{type(e).__name__}: {e.__str__()}")
    ...
    def __handleTypeErrorException(self, query) -> DBResponse:
        ...
        if "%" in query:
            ...
            raise Exception(
                "58000 - system_error: Waduh, ada error baru yang belum pernah ditemui, semangat!"
            )

    def __logprint_exception(
        self, e: PsycopgError, dept_level: int = 3, print_query: bool = True
    ) -> None:
        """
    ...
    def execute(
        ...
        try:
            ...
            e = self.__serialize_exception(e)
            not suppress_error and self.__logprint_exception(e)

            return self.__response(e.pgcode, e.pgerror, e.diag)
        finally:
            self.__flag_execute_biasa = True
            if ec.status:
                self.__release_connection()

    def execute_dt(
        self, query: str, param: dict = {}, limit: int = 10, print_query: bool = False
    ) -> DBResponse:
        """
        ...
        try:
            ...
            query = query.replace(";", "")
            query_count = f"SELECT COUNT(*) AS total FROM ({query}) AS total;"

            # Karena DT server side maka wajib ada limit & offset (kita tambahkan otomatis)
            # NOTE: kalau user sudah kasih limit offset secara manual, bakal error :)
    ...
    def execute_many(
        self, query: str, listData: list, print_query: bool = False
    ) -> DBResponse:
        """
        ...
        try:
            ...
            print(
                "Hint: execute_many pakai %s untuk param-nya: 'INSERT INTO table_x VALUES %s;"
            )
    ...
    def execute_preserve(
        ...
        try:
            ...
            self.__result_preserve = self.__response("00000", None)
            return self.__result_preserve
        except TypeError:
            self.__release_connection()
            return self.__handleTypeErrorException(query)
        except Exception as e:
    ...
    def execute_many_preserve(
        self, query, listData, print_query: bool = False
    ) -> DBResponse:
        """
        NOTE:
            - khusus untuk method execute_preserve & execute_many_preserve WAJIB commit() diakhir,
    ...
    def custom_error(
        self, error: Union[Exception, str] = DEFAULT_ERR_MESSAGE, pgcode: str = "58000"
    ) -> DBResponse:
        """
        Method untuk return explicit error (error yg kita tentukan / buat sendiri) dalam bentuk DBResponse obj.
        Contoh: operasi DB berhasil, namun ada fungsi auto email yg gagal.
        """
        error = self.__serialize_exception(error)
        self.__logprint_exception(error, print_query=False)
        return DBResponse(
            pgcode=pgcode, pgerror=error.pgerror, diag=VOID_DIAG, custom_error=True
        )
