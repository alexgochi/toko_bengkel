...
function getTr(event) {
  ...
  return tr.classList.contains("child") ? tr.previousElementSibling : tr;
}

function removeFormAction() {
  /*
        Remove element when current_user.roles don't have the specified roles on attr `acc-roles`
    */
  document.querySelectorAll("[acc-roles]").forEach((element) => {
    const acc_roles = element.getAttribute("acc-roles").split(" ");

    if (!acc_roles.some((ar) => current_user.roles.includes(ar))) {
      element.remove();
    }
  });
}
...
*/
String.prototype.extractPayload = function () {
  return this.split("$.$")[0].replace(/\\"/g, '"').slice(1, -1);
};
...
function ajaxErrorHandler(response) {
  ...
  if (response.readyState == 0) {
    ...
    const { errorType, result } = responseJSON;
...
function scrollEndRow(id_tabel) {
  document
    .querySelector(`#${id_tabel}_wrapper .dataTables_scrollBody`)
    .scrollBy({
      top: 10000,
      behavior: "smooth",
    });
...
function esc_close_modal() {
  ...
  const kecuali = (modal_element) => {
    ...
    if (
      className.some((className) => modal_element.classList.contains(className))
    )
      return true;
  };
...
/**
 * Enables a DataTables search input to trigger a specified function on 'Enter' key press or button click.
 *
 * @param {string} id_search - The ID of the <input> element used for searching in DataTables.
 * @param {function} func_dt_fill - The function to be called when the 'Enter' key is pressed or the search button is clicked.
 */
function dt_search(id_search, func_dt_fill) {
  // biar nggak tiap kali pakai dt_server harus buat listener-nya mulu :)
  $("#" + id_search)
    .on("keyup", (e) => {
      if (e.key === "Enter") func_dt_fill();
    })
    .next()
    .click(() => {
      func_dt_fill();
...
function moveDTSearch(idTabel, direction) {
  ...
  try {
    ...
    messageAlert(
      "W",
      "Gagal memindahkan search dataTable: Tabel tidak ditemukan!"
    );
    // hideLoading();
  }
...
function getInitials(fullName) {
  ...
  for (let i = 0; i < split_name.length; i++) {
    if (i == 0) {
      ...
      format_name.push(split_name[i][0].toUpperCase());
    }
  }
